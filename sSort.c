...
/* sort by SSN */
void shellsort_ssn(MyRecord* records, int size);

/* sort by income */
void shellsort_income(MyRecord* records, int size);

/* sort by first name */
void shellsort_fname(MyRecord* records, int size);

/* sort by last name*/
void shellsort_lname(MyRecord* records, int size);

/* array of function pointers for quick acces to different versions of sort*/
void (*func_ptr[4])(MyRecord* r, int s) = {shellsort_ssn, shellsort_fname,  shellsort_lname, shellsort_income};
...
int main(int argc, char *argv[], char *envp[])
{
    ...
    if(fprintf(out_file, "SH took %lf sec, and used CPU for %lf sec. Range: %d-%d\n", (t2-t1)/ticspersec, cpu_time/ticspersec, list.beg, list.end) < 0)
    {
...
void shellsort_ssn(MyRecord* records, int size)
{
   ...
   for(gap = size/2; gap > 0; gap = gap/2)
   {
       for(i = gap; i < size; i++)
       {
           ...
           {
               if(records[j+gap].ssn > records[j].ssn)
               break;
               else
               {
...
void shellsort_income(MyRecord* records, int size)
{
   ...
   for(gap = size/2; gap > 0; gap = gap/2)
   {
       for(i = gap; i < size; i++)
       {
           for(j = i-gap; j >= 0; j -= gap)
           {
               if(records[j+gap].income > records[j].income)
               break;
               else
               {
                   swap(&records[j],&records[j+gap]);
...
void shellsort_fname(MyRecord* records, int size)
{
   ...
   for(gap = size/2; gap > 0; gap = gap/2)
   {
       for(i = gap; i < size; i++)
       {
           for(j = i-gap; j >= 0; j -= gap)
           {
               if(strcmp(records[j+gap].FirstName, records[j].FirstName) > 0)
               break;
               else
               {
                   swap(&records[j],&records[j+gap]);
...
void shellsort_lname(MyRecord* records, int size)
{
   ...
   for(gap = size/2; gap > 0; gap = gap/2)
   {
       for(i = gap; i < size; i++)
       {
           for(j = i-gap; j >= 0; j -= gap)
           {
               if(strcmp(records[j+gap].LastName, records[j].LastName) > 0)
               break;
               else
               {
                   swap(&records[j],&records[j+gap]);
